...
public class ChatListener implements Listener {
    ...
    public void onAsyncPlayerChat(AsyncPlayerChatEvent event) {
        ...
        // Fix the empty message
        if (finalChatMode == ChatMode.PUBLIC && event.getMessage().trim().length() < 1) {
            event.getPlayer().sendMessage(ChatColor.RED + "You can't send empty messages!");
            return;
        }
