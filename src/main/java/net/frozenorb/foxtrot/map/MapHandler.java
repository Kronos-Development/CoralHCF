...
public class MapHandler {
    ...
    public void load() {
        ...
        if (isKitMap() || Foxtrot.getInstance().getServerHandler().isVeltKitMap()) {
            ...
            purgatory = Foxtrot.getInstance().getConfig().getBoolean("purgatory", false);

            if (purgatory)
                purgatoryHandler = new PurgatoryHandler();
            else
                Foxtrot.getInstance().getServer().getPluginManager().registerEvents(new DeathbanListener(), Foxtrot.getInstance());
        }
    }
